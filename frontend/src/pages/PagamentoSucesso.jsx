...
export default function PagamentoSucesso() {
  ...
  const resumo = useMemo(() => {
    ...
    const cliente = pedido.cliente?.nome ?? pedido.nomeCliente ?? pedido.cliente ?? '—'
    ...
    <div className="bg-[#f7f9ff] min-h-screen py-10">
      <div className="container-max max-w-3xl mx-auto px-4">
        ...
        <div className="bg-white rounded-2xl border border-[#e5e8ee] p-6 mb-6">
          ...
          {loading ? (
            <p className="text-sm text-[#43474f]">Carregando dados do pedido...</p>
          ) : erro ? (
            <p className="text-sm text-[#b42318]">{erro}</p>
          ) : (
            <div className="space-y-2 text-sm text-[#43474f]">
              <p>
                <span className="font-semibold text-[#003366]">Pedido:</span> {resumo.numero}
              </p>
              <p>
                <span className="font-semibold text-[#003366]">Cliente:</span> {resumo.cliente}
              </p>
              <p>
                <span className="font-semibold text-[#003366]">Valor:</span> {resumo.valor}
              </p>
              <p>
                <span className="font-semibold text-[#003366]">Pagamento:</span> {resumo.pagamento}
              </p>
              <p>
                <span className="font-semibold text-[#003366]">Data:</span> {resumo.data}
              </p>
            </div>
          )}
        </div>
